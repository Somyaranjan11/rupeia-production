...
const CustomerReview = () => {
  ...
  return (
    <div className="px-5 pb-10">
      <p className="text-[22px] sm:text-[42px] my-16 text-center font-semibold bg-gradient-to-r from-[#d0cccb] via-[#6a6867] to-[#d0cccb] bg-clip-text text-transparent">
      ...
      <div className="w-full flex justify-start sm:justify-center items-center overflow-x-auto sm:px-28">
        <div className="flex gap-4 sm:gap-14 px-2">
          {customerReview?.map((data, index) => (
            <div
              key={index}
              className="h-[400px] min-w-[250px] sm:min-w-0 w-[428px] bg-[#D9D9D914] border border-[#FFFFFF61] px-5 py-10 gap-5 rounded-xl flex  items-center flex-col"
            >
              ...
              <div className="flex flex-col gap-[0.5px] w-full">
                <p className="text-[16px] text-[#6A6867] font-bold">
                  {data?.name}
                </p>
                <p className="text-[16px] text-[#6A6867] font-bold">
                  {data?.details_1}

...
const OurValue = () => {
  ...
  return (
    <div className="px-5 sm:px-28 py-5 sm:py-10 flex flex-col gap-5 sm:gap-10 ">
      <div>
        <p className="text-[#FFFFFF] text-[24px] sm:text-[48px] font-medium text-center">
          The Values That Drive Us
        </p>
      </div>
      <div className="flex flex-row gap-3 sm:gap-7 sm:flex-wrap justify-start sm:justify-center items-center overflow-x-auto hide-scrollbar">
        {dummyValueData?.map((data, index) => (
          <div
            key={index}
            className="p-4 sm:p-10 bg-[#D9D9D9] min-w-[260px] sm:min-w-0 sm:w-[400px] h-[260px] flex gap-2 justify-center items-center flex-col rounded-3xl"
          >

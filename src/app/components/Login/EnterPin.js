...
const EnterPin = ({ setPage, page }) => {
  const inputsRef = [useRef(), useRef(), useRef(), useRef()];
  const [verifyData, setVerifyData] = useState({ pin: "" });
  ...
  const handleChange = (e, index) => {
    ...
    const newPin = [...(verifyData.pin || ["", "", "", ""])];
    newPin[index] = value;

    setVerifyData({ ...verifyData, pin: newPin });
  ...
  const handleContinue = async () => {
    const pin = (verifyData.pin || []).join("");
    if (pin < 5) {
      ShowErroemessage("Please enter a 4-digit PIN");
      return;
    } else {
      const payloadData = {
        pin: verifyData?.pin.join(""),
      };
  ...
  return (
    <div className="px-5 pt-28 flex flex-col gap-5 w-full">
